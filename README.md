...
1. **本机开房（推荐，手机可用）**：进联机大厅 → 点「🏠 本机开房(朋友填IP直连)」→
   大厅会显示本机局域网 IP → 朋友在大厅右上「服务器」填这个 IP 点「连接」→
   输入房主创建的房间码加入。同一进程内嵌权威服务器，Windows/安卓都能当主机。
   **跨城市/不同网络**：双方安装 [Tailscale](https://tailscale.com)（免费），
   登录后互填 100.x.x.x 虚拟 IP 即可，效果等同同一局域网。
   大厅右下「? 联机帮助」有四页图文说明。
