...
const PAGES := [
	...
	["革命", "打出【四条】立刻触发革命: 点序大小反转!\n3 变成最大, 2 变成最小; 王依然大, ♠3 永远最强。\n四条还是炸弹, 能压任何牌型。劣势局的翻盘号角!", 4],
...
func _build_fig(kind: int) -> void:
	...
	match kind:
		...
		4:
			...
			_mini(51, Vector2(420, 40), 0.95)
			_fig_label("2 最弱", Vector2(416, 150), AppTheme.DIM, 17)
			_mini(0, Vector2(640, 40), 0.95)
			_fig_label("例外: ♠3 仍最强", Vector2(620, 150), AppTheme.GOLD, 15)
